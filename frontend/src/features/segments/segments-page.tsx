...
import { getSegments } from "@/api/contacts";
...
export function SegmentsPage() {
  ...
  const segments = useQuery({ queryKey: ["segments", limit, offset], queryFn: () => getSegments(limit, offset) });
  const total = segments.data?.total ?? 0;
  return (
    <section className="grid gap-5">
      <div>
        <h2 className="text-2xl font-semibold tracking-normal">Segments</h2>
        <p className="text-sm text-muted-foreground">Rule-based audiences prepared for campaign targeting.</p>
      </div>
      <Card>
        <CardContent className="p-0">
          {segments.isLoading ? (
              ...
              <TableBody>
                {segments.data.items.map((segment) => (
                  <TableRow key={segment.id}>
                    ...
                    <TableCell>{String(segment.rules.operator ?? "AND")}</TableCell>
            ...
            <div className="p-8 text-center text-sm text-muted-foreground">No segments found.</div>
          )}
        </CardContent>
      </Card>
